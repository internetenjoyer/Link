...
class DatePicker {
    ...
    handleDayClick(date) {
        // Bugünden sonraki günleri kontrol et
        const today = new Date();
        ...
        // Bugünden sonraki günleri seçmeyi engelle
        if (clickedDate > today) {
            return;
        }
        
        if (!this.state.tempStartDate || (this.state.tempStartDate && this.state.tempEndDate)) {
            // İlk tarih seçimi veya yeni bir aralık başlatma
            this.state.tempStartDate = new Date(date);
            this.state.tempEndDate = null;
            ...
            // İkinci tarih seçimi
            this.state.tempEndDate = new Date(date);
            
            // Başlangıç tarihi bitiş tarihinden sonra ise tarihleri değiştirme
            if (this.state.tempStartDate > this.state.tempEndDate) {
                const temp = this.state.tempStartDate;
                this.state.tempStartDate = this.state.tempEndDate;
                this.state.tempEndDate = temp;
            }
        ...
        if (this.selectedDateRange) {
            if (this.state.tempStartDate && this.state.tempEndDate) {
                const startStr = this.state.tempStartDate.toLocaleDateString('tr-TR');
                const endStr = this.state.tempEndDate.toLocaleDateString('tr-TR');
                this.selectedDateRange.textContent = `${startStr} - ${endStr}`;
            } else if (this.state.tempStartDate) {
                const startStr = this.state.tempStartDate.toLocaleDateString('tr-TR');
                this.selectedDateRange.textContent = `${startStr} - Seçim yapın`;
            }
        }
...
export class UIManager {
    ...
    init() {
        ...
        // Sayfa yüklendiğinde hiçbir filtre seçili olmadığından, filtreleme butonunu devre dışı bırakma
    ...
    addEventListeners() {
        ...
        this.dateStartInput.addEventListener('change', () => this.updateClearFilterButtonVisibility());
        this.dateEndInput.addEventListener('change', () => this.updateClearFilterButtonVisibility());
        this.brandFilterSelect.addEventListener('change', () => this.updateClearFilterButtonVisibility());
        this.categoryFilterSelect.addEventListener('change', () => this.updateClearFilterButtonVisibility());
        
        // Gönderi tipi filtresi için değişiklik dinleyicisi
        const postTypeFilter = document.getElementById('post-type-filter');
        if (postTypeFilter) {
            postTypeFilter.addEventListener('change', () => this.updateClearFilterButtonVisibility());
        }
    ...
    applyFilters() {
        try {
            ...
            // Hiçbir filtre seçilmediyse uyarı göster ve işlemi durdur
            if (!startDate && !endDate && !brandId && !categoryId && !postTypeId) {
                this.toastManager.warning('Lütfen en az bir filtre seçin.', 'Filtre Uyarısı');
                console.log('Hiçbir filtre seçilmedi, işlem durduruldu.');
                return;
            }
    ...
    clearFilters() {
        try {
            ...
            // Bağlantıları yeniden yükleme (filtresiz)
            this.linkManager.loadLinks();
            ...
            this.toastManager.error('Filtreler temizlenirken bir hata oluştu: ' + error.message);
        }
    }
    ...
    updateClearFilterButtonVisibility() {
        try {
            ...
            // Filtreleme butonunun durumunu da güncelle
            this.updateFilterButtonState(isAnyFilterSelected);
        } catch (error) {
    ...
    handleUserFormSubmit() {
        ...
        // Şifre alanı doldurulmuşsa şifreyi güncelleme
    ...
    handleAdminButtons() {
        ...
        // Bağlantıları dışa aktarma butonu
        const exportLinksBtn = document.getElementById('export-links-btn');
        if (exportLinksBtn) {
            exportLinksBtn.addEventListener('click', () => {
                this.handleExportLinks();
            });
        }
        
        // Aşağıdaki butonlar artık HTML'de bulunmuyor, bu yüzden kontrol ekliyoruz
        
        // Tüm verileri JSON olarak dışa aktarma butonu
        const exportAllJsonBtn = document.getElementById('export-all-json-btn');
        if (exportAllJsonBtn) {
            exportAllJsonBtn.addEventListener('click', () => {
                this.dataManager.exportAllDataAsJSON();
            });
        }
        
        // Bağlantıları JSON olarak dışa aktarma butonu
        const exportLinksJsonBtn = document.getElementById('export-links-json-btn');
        if (exportLinksJsonBtn) {
            exportLinksJsonBtn.addEventListener('click', () => {
                this.dataManager.exportLinksAsJSON();
            });
        }
        
        // Kullanıcıları JSON olarak dışa aktarma butonu
        const exportUsersJsonBtn = document.getElementById('export-users-json-btn');
        if (exportUsersJsonBtn) {
            exportUsersJsonBtn.addEventListener('click', () => {
                this.dataManager.exportUsersAsJSON();
            });
        }
        
        // Markaları JSON olarak dışa aktarma butonu
        const exportBrandsJsonBtn = document.getElementById('export-brands-json-btn');
        if (exportBrandsJsonBtn) {
            exportBrandsJsonBtn.addEventListener('click', () => {
                this.dataManager.exportBrandsAsJSON();
            });
        }
        
        // Kategorileri JSON olarak dışa aktarma butonu
        const exportCategoriesJsonBtn = document.getElementById('export-categories-json-btn');
        if (exportCategoriesJsonBtn) {
            exportCategoriesJsonBtn.addEventListener('click', () => {
                this.dataManager.exportCategoriesAsJSON();
            });
        }
    }
    ...
    updateUIBasedOnAuth() {
        ...
        if (isLoggedIn) {
            ...
            if (addLinkBtn) {
                addLinkBtn.classList.toggle('hidden', !canEdit);
            }
            
            // Kullanıcı rolüne göre toplu işlem butonlarını göster/gizle
            const bulkActions = document.getElementById('bulk-actions');
            if (bulkActions) {
                bulkActions.classList.toggle('hidden', !canEdit);
            }
        }
    }
